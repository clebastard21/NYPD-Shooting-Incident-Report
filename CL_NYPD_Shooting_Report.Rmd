...
```{r prepare_data, indent = "     "}
#Data preparation
shootings_NY_per_boro <- shooting_NY %>% group_by(BORO) %>% summarize(cases = n())
murders_NY_per_boro <- merge(shooting_NY %>% group_by(BORO, STATISTICAL_MURDER_FLAG) %>% summarize(cases = n()),
...
shootings_NY_perp_race_vic_race <- shootings_NY_perp_race_vic_race %>% rename(cases = cases.x, total_cases = cases.y)
shootings_NY_perp_race_vic_race <- shootings_NY_perp_race_vic_race %>% mutate(pct = round(cases / total_cases * 100, 2))
#Perpetrator per sex
...
shootings_NY_perp_race_vic_sex <- shootings_NY_perp_race_vic_sex %>% rename(cases = cases.x, total_cases = cases.y)
shootings_NY_perp_race_vic_sex <- shootings_NY_perp_race_vic_sex %>% mutate(pct = round(cases / total_cases * 100, 2))
```
<br>
